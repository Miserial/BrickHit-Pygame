...
while True:
    ...
    if Hits == NUM_BLOCK_ROWS*NUM_BLOCK_COLUMNS:
    ...
    for i in range(NUM_BLOCK_ROWS):
        ...
        for j in range(NUM_BLOCK_COLUMNS):
            if Blocks[i][j]:
                pygame.draw.rect(screen,BLOCK_COLOR,(cur_x,cur_y,bW,bH))
            cur_x+=x_GAP
        cur_y+=y_GAP
    pygame.draw.circle(screen,BALL_COLOR,(ball_x,ball_y),R)
    pygame.draw.rect(screen,PADDLE_COLOR,(pX,pY,pW,pH))
    # ------------绘制文字----
    titleinfo=font.render_to(screen, (w//2-80,10), "车万灵异传（伪）",fgcolor=WHITE)
