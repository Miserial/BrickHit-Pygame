...
class Block(MyObject):
    ...
    def EmptyBlocks(self,group):
        group.empty()
    def CreateBlocks(self,n_row,n_col,size,offset,COLORS,group,group_special,mask):
...
class Data(object):
    ...
    def add_Score(self,dx,combo):
        GAIN = VELOCITY[abs(dx)] if abs(dx)<=5 else 1.8
        GAIN += COMBO[combo] if combo<=5 else 2
        GAIN -=1
        print(GAIN)
        self.score += (10*GAIN)
...
STATE=-1
VELOCITY={0:1,1:1,2:1,3:1.1,4:1.2,5:1.5}
COMBO={0:1,1:1,2:1.1,3:1.2,4:1.4,5:1.7}
combo=0
TOTAL_LIFE=10
#pygame 初始化
pygame.init()
...
blocks_special=pygame.sprite.Group()
block=Block(1)
block.CreateBlocks(COL,ROW,(28,28),(31,31),COLORS,blocks,blocks_special,Lv[data.level])
for obj in iter(blocks):
    if obj.type==8:
        print(obj.rect.center)
        obj.image=pygame.image.load('blank.png').convert_alpha()
...
while True:
    ...
    if len(blocks)==len(blocks_special):
    ...
    if STATE == STATE_SET['PENDING']:
        ...
        block.EmptyBlocks(blocks_special)
        block.CreateBlocks(COL,ROW, (28, 28), (31, 31), COLORS, blocks,blocks_special,Lv[0])
    ...
    if collision:
        ...
        if collision.type <=0:
            collision.kill()
            data.add_Score(ball.dx, combo)
        elif collision.type ==8:
            try:
                center=[obj.rect.midtop for obj in iter(blocks) if obj.type==8 and obj.rect.center != collision.rect.center ]
                #print(center)
                ball.set_pos(center[0][0],center[0][1]-16)  #if >2?
                ball.set_vel(randint(-2,2),ball.dy)
            except:
                ball.set_pos(collision.rect.centerx,collision.rect.top-16)
                ball.set_vel(randint(-2, 2), ball.dy)
        elif collision.type ==9: pass
        else: data.add_Score(ball.dx,combo)
    ...
    print_text(font_en, border.right - 180, border.top, "VEL:%d COMBO:%d" % (ball.dx, combo))
    pygame.display.update()
    MainClock.tick(126)